...
@jit(nopython=True)
def enthalpyode(time,ye,N,y,dy,phi,Stefan,alpha,beta,G,nu,EffP,Pe,V):
...
if __name__ == "__main__":
    ...
    sol = solve_ivp(lambda t,x: enthalpyode(t,x,mesh.N,mesh.y,mesh.dy,prms.phi,nd.Stefan,prms.alpha,prms.beta,nd.G,nd.nu,nd.EffP,nd.Pe,nd.V),[0,mesh.TotalTime],yei,method='LSODA')
    np.savetxt('mol_esol.txt', (mesh.y,sol.y[:,-1]))
    ...
    heave = theta>=0
    It1 = nd.G*(nd.nu-1)*(1-prms.phi)*(mesh.y[heave][-1]-mesh.y[~heave][-1])
    It2 = (1-prms.phi)*theta[-1] + (prms.phi/(1-prms.beta))*(((1+theta[-1])**(1-prms.beta))-1)
    Ib = np.sum((((1-prms.phi*Sfun(theta,prms.beta))**2)/kfun(theta,prms.alpha))*mesh.dy*heave)
    Ib2 = np.trapz(mesh.y*heave,(((1-prms.phi*Sfun(theta,prms.beta))**2)/kfun(theta,prms.alpha))*heave)
    Vtst = (1-nd.EffP+It1+It2)/Ib
    
    def ffintegral(V,zf): 
        tp = lambda t,x: 1+prms.phi*nd.Pe*nd.V*(1-((1+x)**(-prms.beta)))
    ...
    esol_lower = -(prms.phi/nd.Stefan)*(z_lower-zf)
    plt.plot(esol_lower,z_lower,'r-')
