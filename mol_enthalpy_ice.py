...
class meshinformation():
    def __init__(self):
        ...
        self.r = 2 # nondimensional top
        self.N = (2**8)*(self.r-self.l) # Number of grid cells
        ...
        # # time information # #
        self.TotalTime = 40 # total nondimensional time
...
def Vintegrals(theta,heave,alpha,beta,G,nu,phi,y,dy,EffP):
    It1 = np.sum(G*(nu-1)*(1-phi)*dy*heave)
    It2 = (1-phi)*theta[heave][-1] + (phi/(1-beta))*(((1+theta[heave][-1])**(1-beta))-1)
...
def EffPloc(theta,heave,alpha,beta,G,nu,phi,y,dy,EffP,velocity):
    IG = G*np.cumsum((nu*(1-phi)+phi*(1-Sfun(theta,beta)))*dy*heave)*heave
    Ipsdt = phi*theta*heave + (phi/(1-beta))*(((1+theta*heave)**(1-beta))-1) + phi*Sfun(theta*heave,beta)*(1+theta*heave)
    Iv = velocity*np.cumsum((((1-phi*Sfun(theta,beta))**2)/kfun(theta,alpha))*dy*heave)*heave
...
def enthalpyode(time,ye,N,y,dy,phi,Stefan,alpha,beta,G,nu,EffP,Pe,V,Ks,Ki,Kw,K):
        ...
        heaving = np.logical_and(ye<=0,ye>-phi)
...
if __name__ == "__main__":
    ...
    thetai = mesh.y-0.5
    thetalensi = prms.phi
    yei = temperature2enthalpy(thetai,thetalensi,mesh.N,prms.phi,nd.Stefan,prms.beta)
    ...
    heaving = np.logical_and(sol.y[:,-1]<=0,sol.y[:,-1]>-prms.phi)
    V = Vintegrals(theta,heaving,prms.alpha,prms.beta,nd.G,nd.nu,prms.phi,mesh.y,mesh.dy,nd.EffP)
    print(V) # V value
    
    # # lowest ice lens height # #
    zl = max(mesh.y[heaving]) # middle of domain (ice above)
    
    It1 = np.sum(nd.G*(nd.nu-1)*(1-prms.phi)*mesh.dy*heaving)
    It2 = (1-prms.phi)*theta[heaving][-1] + (prms.phi/(1-prms.beta))*(((1+theta[heaving][-1])**(1-prms.beta))-1)
    Ib = np.sum((((1-prms.phi*Sfun(theta,prms.beta))**2)/kfun(theta,prms.alpha))*mesh.dy*heaving)
    Vout = (1-nd.EffP+It1+It2)/Ib
    ...
    def ffintegral(V,zf): 
        ...
        sol = solve_ivp(tp,[zf,zl],[0],method='BDF',dense_output=True)
    ...
    zf = fsolve(lambda x: ffintegral(nd.V,x)-nd.V,0.48)
    tp = lambda t,x: (1+prms.phi*nd.Pe*nd.V*(1-((1+x)**(-prms.beta))))/Kfun(x,prms.Ks,prms.Ki,prms.Kw,bracket.K,prms.phi,prms.beta,prms.alpha)
    shoot_sol = solve_ivp(tp,[zf,zl],[0],method='BDF',dense_output=True)
    ...
    upper_sol = solve_ivp(tp,[zl,mesh.r],[theta[heaving][-1]],method='BDF',dense_output=True)
    esol_upper = -(1/nd.Stefan)*upper_sol.y.T-1
    plt.plot(esol_upper,upper_sol.t,'r-')
    f.savefig("enthalpyheightice.pdf")
    plt.show()
    ...
    plt.plot(upper_sol.y.T,upper_sol.t,'r')
    ...
    f.savefig("localEffP_varyKice.pdf")
    plt.show()
    
    # calculate each piece
    IG = nd.G*np.cumsum((nd.nu*(1-prms.phi)+prms.phi*(1-Sfun(theta,prms.beta)))*mesh.dy*heaving)*heaving
    ...
    Iv = V*np.cumsum((((1-prms.phi*Sfun(theta,prms.beta))**2)/kfun(theta,prms.alpha))*mesh.dy*heaving)*heaving
    Nloc_ep = nd.EffP*heaving-IG-Ipsdt+Iv
    f = plt.figure()
    plt.plot(Nloc_ep/nd.EffP,mesh.y*bracket.z,'k.-')
    plt.rc('text', usetex=True)
    plt.rc('font', family='serif')
    plt.ylim(0,3.5)
    plt.xlim(0,1)
    ...
    plt.xlabel('scaled local effective pressure, $N_{\\textrm{loc}}/N$')
    f.savefig("localEffP_varyKice_dimensional.pdf")
    plt.show()
