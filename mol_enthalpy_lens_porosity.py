...
@jit(nopython=True)
def dalembert(porosity,V,T,y,dy,zl):
    dt = dy/(3*abs(V))
    Nt = round(T/dt)
    zv = zl
    for j in range(Nt):
        ...
        if abs(zv-zl)>dy:
            if V<0:
                pa = porosity[above]
                porosity[above] = np.append(pa[1:], pa[-1])
                zv = zl
            elif V>0:
                pa = porosity[above]
                porosity[above] = np.append(pa[0], pa[0:-1])
                zv = zl           
    return porosity
...
if __name__ == "__main__":
    ...
    z = 2*mesh.y
    dz = 2*mesh.dy
    porosity = 1*(z>7)+0.35*(z<=7)
    porosity[(z>9)*(z<10)]=0.35
    ...
    for i in range(numlens):
        ...
        # porosity plot
        nzl = mesh.y[(min(Nloc[heaving])==Nloc)*heaving]
        porosity = dalembert(porosity,nd.V,max(sol.t),z,dz,min(z[porosity==1]))
        plt.plot(porosity,z)
        zpl = min(z[porosity==1]) - (mesh.r-nzl)
        porosity[min(abs(z-zpl)) == abs(z-zpl)]=1
        
        print(i)
        print(sol.t_events)
        
    f = plt.figure()
    plt.plot(porosity,z,'k.-')
    plt.rc('text', usetex=True)
    plt.rc('font', family='serif')
    plt.ylim(mesh.l,2*mesh.r)
    plt.xlim(0.2,1.2)
    plt.ylabel('height, $z$')
    plt.xlabel('porosity, $\phi$')
    f.savefig("lens_porosity.pdf")