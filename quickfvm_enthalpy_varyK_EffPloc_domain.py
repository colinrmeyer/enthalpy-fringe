...
class parameters():
    def __init__(self):
        ...
        self.EffP = 100*1000; # effective pressure, Pa
...
class nondimensional():
    def __init__(self):
        ...
        self.Qout = 1.8; # qout/qin, nd flux through top
...
if __name__ == "__main__":
    ...
    def ffintegral(zf): 
        ...
        sol = solve_ivp(tp,[zf,zl],[0],method='LSODA',dense_output=True)
    ...
    tp = lambda t,x: (nd.Qout + ((1-nd.Qout)/((1+x)**prms.beta)))/Kfun(x,prms.Ks,prms.Ki,prms.Kw,bracket.K,prms.phi,prms.beta,prms.alpha)
    sol = solve_ivp(tp,[zf,zl],[0],method='LSODA',rtol=1e-6,dense_output=True)
    ...
    plt.plot([0,0],[0,zl],'k-',linewidth=1)
    plt.rc('text', usetex=True)
    plt.rc('font', family='serif')
    plt.ylim(0,zl)
    plt.xlim(-0.2,1.2)
